...
B_rot_H_text = ax_dict["plot"].text(0.7, 0.2, r"$H' \; =$", color = "black", alpha = 0, size = 18)
...
B_rot_H_zeeman_eq = ax_dict["plot"].text(0.95, 0.2, r"$( \omega_L - \omega )  S'_z + $", color = vector_colors[0], alpha = 0, size = 18)
B_rot_H_drive_eq = ax_dict["plot"].text(1.6, 0.2, r"$h S'_x$", color = vector_colors[1], alpha = 0, size = 18)
...
W_trans_equation = ax_dict["plot"].text(-0.1, 0.05, r"$W = \mathrm{exp}( - i \omega t S_z)$", color = "black", alpha = 0, size = 17)
...
tail = int( np.ceil( np.pi/2 * len(spin_precession_phi) / np.abs(spin_precession_phi[-1])) )
# print(tail)
...
def animate(i):
    ...
    if i <= t_0:
        ...
        sphere_dict["bloch_rot"].vector_alpha = [new_alpha, new_alpha, new_alpha]
    ...
    if t_0 < i <= t_9:
        ...
        sphere_dict["bloch_rot"].add_vectors( [B_zeeman_rot[B_time_index], B_drive_rot[B_time_index], spin_rot_frame[B_time_index] ])
