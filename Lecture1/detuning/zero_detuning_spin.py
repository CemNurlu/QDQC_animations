...
spin_precession_phi = np.linspace(0, 13.3*np.pi, len(phi))
...
B_resultant_lab = B_zeeman_lab + B_drive_lab

B_rot_H_text = ax_dict["plot"].text(1.2, 0.2, r"$H' \; =$", color = "black", alpha = 0, size = 18, math_fontfamily= math_fontfamily)
B_rot_H_drive_eq = ax_dict["plot"].text(1.45, 0.2, r"$h S'_x$", color = vector_colors[1], alpha = 0, size = 18, math_fontfamily= math_fontfamily)

B_lab_H_text = ax_dict["plot"].text(-1.55, 0.2, r'$H \; =$', color = "black", alpha = 0, size = 18, math_fontfamily= math_fontfamily)
B_lab_H_zeeman_eq = ax_dict["plot"].text(-1.35, 0.2, r'$\omega_L S_z + $', color = vector_colors[0], alpha = 0, size = 18, math_fontfamily= math_fontfamily)
...
omega_is_omega_L = ax_dict["plot_between"].text(0.25, 0.7, r'$\omega = \omega_L$', color = "black", alpha = 1, size = 30, math_fontfamily = math_fontfamily)
...
def animate(i):
    ...
    if t_3 < i <= t_4:
        ...
        sphere_dict["bloch_rot"].vector_alpha = [new_alpha_comp, new_alpha_res, 0]
        sphere_dict["bloch_rot"].vectors = []
        sphere_dict["bloch_rot"].add_vectors( [B_drive_rot[B_time_index], B_drive_rot[B_time_index]])
        sphere_dict["bloch_rot"].make_sphere()
