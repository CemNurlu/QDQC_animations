...
def animate(i):
    ...
    if t_4 < i <= t_5: #Fade out levels
        pass
    
    if t_5 < i <= t_6: #Fade in Spins
        new_alpha = 1 - (i - t_5) / (t_6 - t_5)
        Spin_1_level_down.alpha = new_alpha
        Spin_1_level_up.alpha = new_alpha
        Spin_2_level_down.alpha = new_alpha
        Spin_2_level_up.alpha = new_alpha
    
    if t_6 < i <= t_7: #Fade in s1 field lines
        alpha = (i - t_6) / (t_7 - t_6)
        spin1.line_alpha = alpha
    
    if t_7 < i <= t_8:#Fade out s1 field lines
        alpha = 1 - (i - t_7) / (t_8 - t_7)
        spin1.line_alpha = alpha
    
    if t_8 < i <= t_9:#Fade in s2 field lines
        alpha = (i - t_8) / (t_9 - t_8)
        spin2.line_alpha = alpha

    if t_9 < i <= t_10:#Fade out s2 field lines
        alpha = 1 - (i - t_9) / (t_10 - t_9)
        spin2.line_alpha = alpha
...
cache_then_save_funcanimation(ani, f'animations/test/shaking_setup_picture.{file_type}', fps = 30)
