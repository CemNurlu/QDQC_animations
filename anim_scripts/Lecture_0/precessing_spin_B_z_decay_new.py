...
t_4 = 300 # Move S_x plot & S_z
t_5 = 320 # Show T2 arrow and envelope & T1 arrow and envelope
t_6 = 370 # Do nothing
t_7 = 390 # Hide Sz graph
t_8 = 410 # Do nothing
t_9 = 430 # Show fourier transform and 1/T2 arrow
t_10 = N_time
...
theta_end_spin = np.pi/12
tau = np.log(theta_end_spin/theta_0_spin)


exp_envelope = np.exp(tau*B_time)
exp_envelope_z = np.exp(tau*B_time*2)

z_spin = np.cos(theta_0_spin)*exp_envelope_z
rho_spin = np.sin(theta_0_spin)*exp_envelope

theta_spin = np.arctan(rho_spin/z_spin)
phi_spin = np.linspace(np.pi/3, np.pi*(10+1/3), t_2-t_1)
r_spin = z_spin/np.cos(theta_spin)

bloch_points_spin = bloch_vector(theta_spin, phi_spin, r_spin)
...
T2_text = ax_dict["plot_0"].text(0.18, -0.05, r'$ \sim 2T_1$', size = 25, alpha = 0, math_fontfamily = math_fontfamily)

Sx_Sz_margin = 4.3

T1_text = ax_dict["plot_0"].text(0.09, -0.05 - Sx_Sz_margin, r'$ \sim T_1$', size = 25, alpha = 0, math_fontfamily = math_fontfamily)

T1_arrow = FancyArrowPatch((0.0, -0.15 - Sx_Sz_margin), (0.5, -0.15 - Sx_Sz_margin), 
        arrowstyle='<->', mutation_scale=20,
        lw = 1.5, color = "black",
        alpha = 0
    )
...
T2_inv_text = ax_dict["plot_0"].text(1, -5.9, r'$ \sim \frac{1}{2T_1}$', size = 25, alpha = 0, math_fontfamily = math_fontfamily)

for arrow in [T2_arrow, T1_arrow, T2_inv_arrow]:
...
def animate(i):
   if i <= t_0:
      ...
      new_x_pos = (1.4 * (1-new_alpha),
                   2.4, 
                   -1.2 - 0.6*new_alpha)
      new_y_pos = (-2.2 - 1.2*new_alpha, 
               - 0.2,
                1.4 * (1-new_alpha))
      ...
      new_x_Szpos = (1.4 * (1-new_alpha),
                   2.4, 
                   -6*(1-new_alpha) - 5.4*new_alpha)
      new_y_Szpos = (-7, 
               -5*(1-new_alpha) - 3.8*new_alpha,
                1.4 * (1-new_alpha))
      S_z_pretty_axis = pretty_axises[2]
      S_z_pretty_axis.update_x_y_pos(new_x_Szpos, new_y_Szpos)

   elif i <= t_5:
      if i == t_4 + 1:
         pretty_axises[0].add_line("pos_env", B_time, rho_spin[0]*exp_envelope, c = "silver", linestyle = "--", alpha = 0, lw = 1.5)
         pretty_axises[0].add_line("neg_env", B_time, -rho_spin[0]*exp_envelope, c = "silver", linestyle = "--", alpha = 0, lw = 1.5)
      new_alpha = (i-t_4)/(t_5-t_4)
      T2_arrow.set_alpha(new_alpha)
      T2_text.set_alpha(new_alpha)
      T1_arrow.set_alpha(new_alpha)
      T1_text.set_alpha(new_alpha)
      ...
      new_alpha = 1-(i-t_6)/(t_7-t_6)
      T1_arrow.set_alpha(new_alpha)
      T1_text.set_alpha(new_alpha)
      pretty_axises[2].alpha = new_alpha
   
   elif i <= t_8:
      pass

   elif i <= t_9:
      new_alpha = (i-t_8)/(t_9-t_8)
   ...
   B_text = ax_dict["bloch_0"].text(0.05,0,0.85,r'$ B(t)$', size = 20, alpha = 1,
...
cache_then_save_funcanimation(ani, f'animations/test/precessing_spin_B_Z_decay_new.{file_type}', fps = 20 )