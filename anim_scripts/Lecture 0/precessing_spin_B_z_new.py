...
t_2 = 320 # Show time evolution
t_3 = 350 # remove plots except S_x
t_4 = 370 # Move S_x plot
t_5 = 450 # Show fourier transform
t_6 = N_time
...
n_turns = 10
phi_initial = 0
phi_spin = np.linspace(phi_initial, np.pi*(n_turns+phi_initial), t_2-t_1)
...
if n_component_plots > 0:
   ...
   #  pretty_axis_B.add_label(r" $t$", "x", size = 25)
...
LarmorPrec_text = ax_dict["bloch_0"].text(-0.5,0,-1.75,"Larmor Precession", size = 20, alpha = 0, math_fontfamily = math_fontfamily)
B_text = ax_dict["bloch_0"].text(-0.5,0,-1.75,r'$ B(t)$', size = 20, alpha = 0, math_fontfamily = math_fontfamily, color = "red")
...
def animate(i):
   if i <= t_0:
      ...
      for coord, include_plot in plot_S_xyz.items():
         if include_plot:
               ...
               if coord == "S_x":
                  component_values = S_x[:S_index]
               elif coord == "S_y":
                  component_values = S_y[:S_index]
               elif coord == "S_z":
                  component_values = S_z[:S_index]

               pretty_axises[plot_index].update_line(coord, B_time[:S_index], component_values)

               plot_index += 1
      pretty_axis_B.update_line("B", B_time[:S_index], np.ones(S_index)*0.7)
      
      sphere_dict["bloch_0"].vectors = sphere_dict["bloch_0"].vectors[0:1]
      sphere_dict["bloch_0"].points = []
      # new_vec = bloch_vector(theta[i], phi[i])
      sphere_dict["bloch_0"].add_vectors(bloch_points_spin[S_index-1])

      if S_index < tail or tail == -1:
         points = bloch_points_spin[:S_index]
      else:
         points = bloch_points_spin[S_index-tail:S_index]
      if S_index > 0:
   ...
   sphere_dict["bloch_0"].make_sphere()
   LarmorPrec_text2 = ax_dict["bloch_0"].text(-0.5,0,-1.75,"Larmor Precession", size = 20, alpha = 1, math_fontfamily = math_fontfamily) 
   B_text = ax_dict["bloch_0"].text(0.05,0,0.75,r'$ B(t)$', size = 20, alpha = 1,
...
cache_then_save_funcanimation(ani, f'animations/test/precessing_spin_B_Z_new.{file_type}', fps = 20 )