...
omega_slider = PrettySlider(ax_dict["plot_between"], 
                x_pos=(0.1, 1), y_pos=(0.7, 0.7), data_lim = (-omega_max*0.1, omega_max*1.1),
                arrow_style="->", slider_dot_data=0, alpha = 0, c = ("black", "aquamarine"), 
                labels = (r"$0$ ", r"$\omega$",None), arrow_lw=3,
                label_size = 20, 
                label_c = ("black", "black", "red"))

line_zero_detuning = lines.Line2D([0.55,0.55],[0.675,0.725], lw = 3, color = "red",alpha = 0)
ax_dict["plot_between"].add_line(line_zero_detuning)
text_zero_detuning = ax_dict["plot_between"].text(0.55, 0.75, r"$\omega_{L}$", color = "red", alpha = 0, size = 20, math_fontfamily = math_fontfamily, ha = "center")
