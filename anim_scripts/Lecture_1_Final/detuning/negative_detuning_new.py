...
B_lab_H_text = ax_dict["plot"].text(-1.3, 0.2, r'$H \; =$', color = "red", alpha = 0, size = 20, math_fontfamily= math_fontfamily)
B_lab_H_zeeman_text = ax_dict["plot"].text(-1.1, 0.2, r'$H_{\mathrm{Zeeman}} + $', color = vector_colors[0], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
...
B_lab_H_zeeman_eq = ax_dict["plot"].text(-1.45, 0.1, r'$=\omega_L S_z + $', color = vector_colors[0], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
B_lab_H_drive_eq = ax_dict["plot"].text(-1.00, 0.1, r"$h [ S_x \: \mathrm{cos}(\omega t) + S_y \: \mathrm{sin}(\omega t) ]$", color = vector_colors[1], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)

B_rot_H_text = ax_dict["plot"].text(0.87, 0.2, r"$H \prime \; =$", color = "red", alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
B_rot_H_zeeman_text = ax_dict["plot"].text(1.12, 0.2, r"$H \prime _{\mathrm{Zeeman}} + $", color = vector_colors[0], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
B_rot_H_drive_text = ax_dict["plot"].text(1.55, 0.2, r"$H \prime _{\mathrm{driving}} \; = \;$", color = vector_colors[1], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
B_rot_H_zeeman_eq = ax_dict["plot"].text(1, 0.1, r"$=( \omega_L - \omega )  S \prime _z + $", color = vector_colors[0], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
B_rot_H_drive_eq = ax_dict["plot"].text(1.65, 0.1, r"$h S \prime _x$", color = vector_colors[1], alpha = 0,  size = 20, math_fontfamily= math_fontfamily)
...
omega_gt_omega_L = ax_dict["plot_between"].text(0.25, 0.7, r'$\omega > \omega_L$', color = "black", alpha = 0, size = 30, math_fontfamily = math_fontfamily)
...
def animate(i):
    ...
    if t_0 < i <= t_13:
        ...
        if t_8 < i <= t_11:
            ...
            B_zeeman_auxvector = (1-coefficent)*B_zeeman_lab[i] + coefficent*B_zeeman_rot[i]
            B_total_auxvector = (1-coefficent)*B_total_lab[0] + coefficent*B_total_rot[i]
            B_zeeman_auxvector = np.sign(B_zeeman_auxvector)*np.maximum(np.abs(B_zeeman_auxvector),arrowLength)
        ...
        if i > t_11:
            sphere_dict["bloch_rot"].vectors = []
            sphere_dict["bloch_rot"].add_vectors([B_zeeman_rot[B_time_index],B_drive_rot[B_time_index],B_total_rot[B_time_index]])
            sphere_dict["bloch_rot"].make_sphere()
...
cache_then_save_funcanimation(ani, f'animations/test/negative_detuning_new.{file_type}', fps = 20 )