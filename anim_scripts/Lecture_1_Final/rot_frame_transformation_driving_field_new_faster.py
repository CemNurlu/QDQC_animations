...
##########################
frames_per_period = 100/3
#N_add_rounds = 2
N_add_rounds = 2
N_time = 620 + 100*N_add_rounds
t_0 = 20 # Show decomposed bloch sphere and pretty axis
t_001= 70 #Subtle Appearence of decomposition
t_001 = t_0 + int(1.5*frames_per_period)
t_01 = 80#Show the decomposition
t_01 = t_001 + int(0.3*frames_per_period)
t_002 = 170 #Subtle Disappearance of the decomposition
t_002 = t_01 + int(2.7*frames_per_period)
t_02 = 180 #Hide B_x total
t_02 = t_002 + int(0.3*frames_per_period)
t_1 = 100 # Show B_dec time evolution
t_1 = t_0 + int(2.4*frames_per_period)
t_2 = 120 # Show B_dec equation
t_2 = t_1 + int(0.6*frames_per_period) 
t_3 = 160 # Let it sink in for a bit
t_3 = t_2 + int(1.2*frames_per_period)
t_4 = 180 # Show W transformation equation
t_4 = t_3 + int(0.6*frames_per_period)
t_5 = 220 # Let it sink in for a bit
t_5 = t_4 + int(1.2*frames_per_period)
t_6 = 320 + 100*N_add_rounds # Show rot sphere and rot field time evolution
t_6 = t_5 + int((N_add_rounds + 3)*frames_per_period)
t_7 = 390 + 100*N_add_rounds # Show static sphere and static + rot field time evolution + pretty_plot
t_7 = t_6 + int(2.1*frames_per_period)
t_8 = 410 + 100*N_add_rounds # Show B_rot equation version 1
t_8 = t_7 + int(0.6*frames_per_period)
t_9 = 460 + 100*N_add_rounds # Let it sink in for a bit
t_9 = t_8 + int(1.5*frames_per_period)
t_10 = 480 + 100*N_add_rounds# Show B_rot equation version 2
t_10 = t_9 + int(0.6*frames_per_period)
t_11 = 520 + 100*N_add_rounds# Let it sink in for a bit
t_11 = t_10 + int(1.2*frames_per_period)
t_12 = 560 + 100*N_add_rounds# Fade out counter rotating arrows and equation
t_12 = t_11 + int(1.2*frames_per_period)
t_13 = 620 + 100*N_add_rounds# Show time evolution of B_rot
t_13 = t_12 + int(1.8*frames_per_period)
N_time = t_13
t_14 = N_time
...
vector_colors = ["purple", "hotpink", "deepskyblue","purple"]#, "blue", "green", vector_colors[0], "orange", "pink", "yellow"]
...
B_x_max = 1
phi_0 = 0
phi_end = 2*int(18 + 3*N_add_rounds)*np.pi
#phi_end = 2*int(18 + N_add_rounds)*np.pi
...
B_rot = np.zeros((3,t_13-t_0, 3))
B_rot[0,:,0] = B_x_max
B_rot[1,:,0] = B_x_max * np.cos(4*phi)
B_rot[1,:,1] = - B_x_max * np.sin(4*phi)
B_rot[2,:,0] = B_x_max * np.cos(2*phi)
B_rot[2,:,1] = - B_x_max * np.sin(2*phi)
...
pretty_axis_B_rot.add_label(r"$B \prime _{x}(t)$  ", "x", size = 25)
pretty_axis_B_rot.add_label(r"$B \prime _{y}(t)$  ", "y", size = 25)
...
B_dec_H_equation = ax_dict["plot"].text(0, 2.05, r'$H_{driving}\; = \;$', color = vector_colors[0], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
B_dec_0_equation = ax_dict["plot"].text(0, 1.65, r'$=\; h [ S_x \: \mathrm{cos}(\omega t) + S_y \: \mathrm{sin}(\omega t) ]\; +$', color = vector_colors[1], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
B_dec_1_equation = ax_dict["plot"].text(0, 1.25, r'$+ \; h [ S_x \: \mathrm{cos}(\omega t) - S_y \: \mathrm{sin}(\omega t) ]$', color = vector_colors[2], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")

B_rot_H_equation = ax_dict["plot"].text(4.1, 2.05, r"$H \prime_{driving}  \; = \;$", color = vector_colors[0], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
B_rot_0_equation = ax_dict["plot"].text(4.1, 1.65, r"$ = \; h S \prime_{x}   $", color = vector_colors[1], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
B_rot_1_equation_harmonic = ax_dict["plot"].text(4.1, 1.25, r"$+ \; h [ S \prime _{x} \mathrm{cos}(2 \omega t) - S \prime _{y} \mathrm{sin}(2 \omega t) ]$", color = vector_colors[2], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
B_rot_1_equation_neglect = ax_dict["plot"].text(4.1, 1.25, r"+ $rapidly \; oscillating \; term$", color = vector_colors[2], alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
...
W_trans_equation = ax_dict["plot_between"].text(0.55, 0.3, r'$W = \mathrm{exp}(-i \omega t S_z)$', color = "black", alpha = 0, size = 30, math_fontfamily = math_fontfamily, ha = "center")
...
def animate(i):
    ...
    if t_0 < i <= t_13:
        ...
        if t_5 < i <= t_6:
            ...
            sphere_dict["bloch_rot"].add_vectors([B_rot[0,B_time_index],B_rot[2,B_time_index]])
            azim_index = i - t_5 - 1
