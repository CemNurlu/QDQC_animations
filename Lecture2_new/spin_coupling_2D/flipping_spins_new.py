...
def animate(i):
    ...
    if t_9 < i <= t_10:#Flip S2
        new_rotation = np.pi*(i-t_9)/(t_10-t_9)
    ...
    if t_12 < i <= t_13: #Fade out spin 1 original levels and fade in new one2
    ...
    if t_14 < i <= t_15: #Fade out new S1 levels and rotate S2 back 
        new_alpha = (i - t_14 ) / (t_15 - t_14)
        new_rotation = np.pi*(1-new_alpha)
        spin2.rotation = new_rotation
        New_1_level_down.alpha = 1 - new_alpha
        New_1_level_up.alpha = 1- new_alpha
    ...
    if t_18 < i <= t_19: #Fade in S2 Original Energy Levels
    ...
    if t_22 < i <= t_23: #Flip S1
        new_rotation = np.pi*(i-t_22)/(t_23-t_22)
    ...
    if t_25 < i <= t_26: #Fade out spin 2 original levels and fade in new ones
        new_alpha = (i-t_25)/(t_26-t_25)
        New_2_level_down.alpha = new_alpha
        New_2_level_up.alpha = new_alpha
        Spin_2_level_up.alpha = 1 - new_alpha
        Spin_2_level_down.alpha = 1 - new_alpha
    ...
    if t_27 < i <= t_28: #Fade out new S2 levels and rotate S1 back 
        new_alpha = (i - t_27 ) / (t_28 - t_27)
        new_rotation = np.pi*(1-new_alpha)
        spin1.rotation = new_rotation
        New_2_level_down.alpha = 1 - new_alpha
        New_2_level_up.alpha = 1- new_alpha
...
cache_then_save_funcanimation(ani, f'animations/test/flipping_spins_new.{file_type}', fps = 30)
